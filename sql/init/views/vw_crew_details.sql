...
CREATE OR REPLACE VIEW vw_crew_details AS
    SELECT
        ...
        ROW_NUMBER() OVER(PARTITION BY sk.cruise_id, sk.boat_id ORDER BY sk.boat_id, sk.skipper_full_name, p.id) AS member_number,
    ...
    ORDER BY sk.cruise_start_date, sk.boat_common_name, sk.skipper_full_name, member_number;
